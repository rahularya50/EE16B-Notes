...
Now, recall that when we considered continuous-time systems with coupled differential equations, we were able to separate them into a set of decoupled differential equations by diagonalizing their state matrix. We can do something similar here, by letting $A = V\Lambda V^{-1}$\footnote{We will discuss the case when $A$ is not diagonalizable later in the course.}, and rearranging to obtain
\eqn{
...
where $\tilde{x} = V^{-1}\vec{x}$ and $\tilde{\omega}$ = $V^{-1}\vec{\omega}.
